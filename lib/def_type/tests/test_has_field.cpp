...
TEST_CASE("plain: has_field() finds all plain members by name", "[type_def][typed][has_field]") {
    type_def<PlainDog> t;
    REQUIRE(t.has_field("name"));
    REQUIRE(t.has_field("age"));
    REQUIRE(t.has_field("breed"));
...
TEST_CASE("plain: has_field() returns false for unknown names", "[type_def][typed][has_field]") {
    type_def<PlainDog> t;
    REQUIRE(!t.has_field("nope"));
    REQUIRE(!t.has_field(""));
    REQUIRE(!t.has_field("Name"));
...
TEST_CASE("typed: has_field() rejects meta member names on MetaDog", "[type_def][typed][has_field]") {
    type_def<MetaDog> t;
    REQUIRE(!t.has_field("help"));
    REQUIRE(t.has_field("name"));
    REQUIRE(t.has_field("age"));
...
// ═════════════════════════════════════════════════════════════════════════
// Plain members are now visible (non-meta members auto-discovered)
// ═════════════════════════════════════════════════════════════════════════

TEST_CASE("typed: has_field() finds plain members in MixedStruct", "[type_def][typed][has_field]") {
...
TEST_CASE("plain: has_field() finds all members of PlainPoint", "[type_def][typed][has_field]") {
...
TEST_CASE("typed: has_field() on meta-only struct with zero fields", "[type_def][typed][has_field]") {
    type_def<MetaOnly> t;
    REQUIRE(!t.has_field("endpoint"));
    REQUIRE(!t.has_field("help"));
